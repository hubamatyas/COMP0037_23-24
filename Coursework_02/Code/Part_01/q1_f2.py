...
if __name__ == '__main__':
    ...
    # Range of alpha values, evenly spaced between 0.1 and 1
    ...
    plt.xlabel('Alpha')
    plt.ylabel('MSE')
    plt.title('MSE of TD Learning vs Alpha with Fixed Iterations')
    plt.grid(True)
    plt.show()

    # Alpha, Iterations = np.meshgrid(alpha_values, iteration_counts)
    # Alpha, Iterations_log = np.meshgrid(alpha_values, np.log10(iteration_counts))
    ...
    # ax_log.set_yticks(np.log10(iteration_counts))  # Set ticks at the positions of the log-scaled iterations
    # ax_log.set_yticklabels(iteration_counts)  # But label them with the original iterations values
